...
namespace SimpleFluentTester.UnitTests
{
    public class TestRunReporterTests
    {
        ...
        private static ITestSuiteReporter CreateReporterFromTestCase<TOutput>(Assert<TOutput> assert, TestCase<TOutput> testCase, string? name = null)
        {
            ...
            var runResult = new TestSuiteResult<TOutput>(
                ...
                name,
                0);
            return new DefaultTestSuiteReporter<TOutput>(runResult);
        }

        private class CustomReporterFactory : ITestSuiteReporterFactory
        {
